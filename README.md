...
HTML/CSS/JavaScriptとlocalStorageだけで動く、毎日の学習内容を記録する小さなアプリです。

このリポジトリは、学習ログアプリに育てるための土台として使います。

日付ごとに「今日の調子・集中度」と「今日の学習メモ」を保存できます。保存した記録はブラウザのlocalStorageに残るため、ページを閉じても同じブラウザで再度開けば学習記録を見返せます。
...
集中度の流れでは、最近7件の記録をこのスコアに変換して、SVGの波形グラフとして表示します。
